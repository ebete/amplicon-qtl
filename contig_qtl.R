...
# make metadata -----------------------------------------------------------
chr_metadata <- read.csv("sl23_gmap.csv") %>%
  arrange(chr, pos) %>%
...
# run R/qtl2 --------------------------------------------------------------
in_cross <- read_cross2("sl23_control.yaml", quiet = F)
...
# plotting ----------------------------------------------------------------
y_limit <- max(lod_scores$lod_score, perm_thresh$threshold) * 1.05
...
lod_scores %>%
  ...
  labs(
    title = "Pink flexible ASVs (rand test)",
    x = "Marker",
    y = "",
    fill = "LOD"
  )
